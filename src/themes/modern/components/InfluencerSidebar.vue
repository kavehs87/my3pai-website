<template>
  <aside class="flex flex-col gap-4 p-4 text-slate-800">
    <!-- Intro Video -->
    <IntroVideoPlayer
    ...
    <div class="flex flex-col items-center text-center">
      ...
      <div v-if="!profile?.introVideoUrl" class="relative mb-3">
        <div class="w-24 h-24 rounded-full overflow-hidden border-2 border-white shadow-lg bg-slate-100">
          <img
            v-if="profile?.image"
            :src="profile.image"
            :alt="profile.name"
      ...
      <h1 class="text-xl font-bold text-slate-900">{{ profile?.name || 'Unknown' }}</h1>
      <p v-if="profile?.handle" class="text-xs font-semibold text-indigo-600 mb-0.5">{{ profile.handle }}</p>
      <p v-if="profile?.tagline" class="text-xs text-slate-600 font-medium mb-2">
        {{ profile.tagline }}
      </p>
      <div class="flex flex-col items-center gap-1 text-slate-500 text-xs mb-3">
    ...
    <!-- Stats Section -->
    <div v-if="hasStats" class="bg-slate-50/80 rounded-xl p-3 flex justify-between items-center text-center">
    ...
    <!-- Consultation CTA -->
    <div v-if="consultation && isAvailable" class="bg-slate-900 rounded-xl p-4 text-white shadow-lg shadow-slate-900/10">
      <div class="flex items-start gap-3">
        <div class="w-10 h-10 rounded-lg bg-slate-800 overflow-hidden shrink-0 flex items-center justify-center">
          <Video class="w-5 h-5 text-slate-400" />
        </div>
        <div>
          <h3 class="font-bold text-base leading-tight">{{ consultation.title }}</h3>
          <p class="text-slate-400 text-xs mt-0.5">Video call • {{ consultation.durationMinutes }} mins</p>
        </div>
      </div>
      <div class="mt-3 flex items-center justify-between">
        ...
        <button 
          class="bg-white text-slate-900 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-slate-100 transition-colors"
    ...
    <!-- External Links -->
    <div v-if="profile?.externalLinks?.length" class="grid grid-cols-2 gap-2">
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        class="bg-white border border-slate-100 p-2.5 rounded-lg shadow-sm hover:shadow-md transition-shadow cursor-pointer block"
      >
    ...
    <!-- About Section -->
    <div v-if="bioParagraphs?.length" class="space-y-2">
      <h2 class="text-base font-bold text-slate-900">About me</h2>
      <p v-for="(paragraph, index) in bioParagraphs" :key="index" class="text-xs text-slate-600 leading-relaxed">
        {{ paragraph }}
      </p>
    </div>

    <!-- Skills and Languages -->
    <div v-if="profile?.skills?.length || profile?.languages?.length" class="space-y-4">
      <div v-if="profile?.skills?.length">
        <h2 class="text-base font-bold text-slate-900 mb-2">Skills and languages</h2>
        <h3 class="text-xs font-bold text-slate-700 mb-1.5">Skills</h3>
        <div class="flex flex-wrap gap-1.5">
          <span
            v-for="skill in profile.skills"
            :key="skill"
            class="px-2.5 py-1 bg-indigo-50 text-indigo-700 text-xs font-semibold rounded-full"
          >
            {{ skill }}
          </span>
        </div>
      ...
      <div v-if="profile?.languages?.length">
        <h3 class="text-xs font-bold text-slate-700 mb-2">Languages</h3>
        <div class="space-y-2">
          <div v-for="language in profile.languages" :key="language.id || language.name" class="flex items-center justify-between text-xs">
    ...
    <div v-if="profile?.certifications?.length" class="space-y-3">
      <h2 class="text-base font-bold text-slate-900">Certifications and badges</h2>
      <div class="grid grid-cols-2 gap-2">
        <div
          v-for="cert in profile.certifications"
          :key="cert"
          class="bg-slate-50 p-2 rounded-lg flex items-center gap-2"
        >
          <div class="w-7 h-7 rounded-full bg-white flex items-center justify-center shrink-0 shadow-sm text-indigo-600">
            <component :is="getCertificationIcon(cert)" class="w-4 h-4 text-indigo-500" />
          </div>
    ...
    <div class="space-y-2">
      <button class="w-full py-2 px-3 bg-white border border-slate-200 hover:bg-slate-50 text-slate-800 rounded-lg text-xs font-semibold shadow-sm transition-all flex items-center justify-center gap-1.5">
      ...
      <button
        class="w-full py-2 px-3 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-xs font-semibold shadow-md transition-all flex items-center justify-center gap-1.5"
        @click="isModalOpen = true"
      >
        <Sparkles class="w-3.5 h-3.5 text-yellow-300" />
        Request custom trip
      </button>
      <button class="w-full py-2 px-3 bg-rose-50 border border-rose-100 hover:bg-rose-100 text-rose-600 rounded-lg text-xs font-semibold shadow-sm transition-all flex items-center justify-center gap-1.5">
