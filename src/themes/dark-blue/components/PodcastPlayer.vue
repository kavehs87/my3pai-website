  ...
  <div class="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
    ...
    <div class="space-y-3">
      ...
      <div class="flex items-center gap-2">
        <span class="text-xs text-text-muted font-mono tabular-nums min-w-[3rem] text-right">
        ...
        >
          <div 
            class="h-full bg-secondary rounded-full transition-all duration-100"
      ...
      <div class="flex items-center justify-center gap-3">
        ...
        >
          <Rewind class="w-5 h-5 text-primary" />
        </button>
        
        <button
          @click="togglePlay"
          :disabled="loading || (podcast.isPremium && isLocked && !audioUrl)"
          class="p-3 bg-secondary text-white rounded-full hover:bg-secondary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
        >
        ...
        >
          <FastForward class="w-5 h-5 text-primary" />
        </button>
      </div>
    </div>
...
const fetchAudioUrl = async () => {
  ...
  loading.value = true
  try {
    const result = await api.getInfluencerPodcastEpisode(currentUsername.value, props.podcast.slug)
