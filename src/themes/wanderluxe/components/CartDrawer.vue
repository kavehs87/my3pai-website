    ...
    >
      ...
      <div class="p-6 border-b border-slate-100 flex justify-between items-center bg-white">
        <div class="flex items-center gap-2">
          ...
          <h2 class="text-xl font-bold text-slate-900">Your Basket ({{ cartItems.length }})</h2>
        </div>
        <button
          @click="$emit('close')"
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-4">
        <!-- Loading State -->
        <div v-if="loading" class="h-full flex flex-col items-center justify-center text-slate-400 space-y-4">
        ...
        >
          <div class="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center">
            <ShoppingBasket class="w-8 h-8 opacity-20" />
          </div>
          <p class="text-lg font-medium">Your basket is empty</p>
          <button
        ...
        <div
          ...
          :key="item.id"
          class="flex gap-4 bg-slate-50 rounded-xl p-4 border border-slate-100"
        >
          ...
          <div class="flex-1 min-w-0 flex flex-col justify-between py-1">
            <div>
              <div class="flex items-center gap-2 mb-1">
                <h3 class="font-bold text-slate-900 leading-tight line-clamp-2">{{ getItemName(item) }}</h3>
                <span
              ...
              <div class="flex items-center gap-2 mt-1">
                <span class="text-xs text-slate-500 uppercase font-semibold">{{ getItemTypeLabel(item) }}</span>
                <span
            ...
            <div class="flex justify-between items-center mt-2">
              <div class="flex items-center gap-3">
                ...
                <PriceDisplay :amount="getItemPriceInDollars(item)" class="font-bold text-slate-900" />
              </div>
              <button
                @click="handleRemove(item.id)"
                :disabled="removingItemId === item.id"
                class="text-slate-400 hover:text-red-500 transition-colors p-1 disabled:opacity-50"
              >
      ...
      <!-- Footer -->
      <div v-if="!loading && cartItems.length > 0" class="p-6 border-t border-slate-100 bg-white space-y-4">
        <!-- Sign in message for guests -->
        <div v-if="!isAuthenticated" class="bg-indigo-50 border border-indigo-100 rounded-xl p-4 flex items-center gap-3">
          <div class="flex-1">
            <p class="text-sm font-semibold text-slate-900 mb-1">Sign in to save your cart</p>
            <p class="text-xs text-slate-500">Your items will be saved to your account</p>
          </div>
          <button
            @click="handleSignInClick"
            class="px-4 py-2 bg-indigo-600 text-white rounded-lg font-medium text-sm hover:bg-indigo-700 transition-colors whitespace-nowrap"
          >
            Sign In
          </button>
        </div>
        ...
        <div class="flex justify-between items-end">
          ...
          <PriceDisplay :amount="subtotal" class="text-2xl font-bold text-slate-900" />
        </div>
        <p v-if="subtotal > 0" class="text-xs text-slate-400 text-center">Taxes and shipping calculated at checkout</p>
        ...
        <button
          ...
          class="w-full bg-slate-900 text-white py-4 rounded-xl font-bold text-lg hover:bg-slate-800 transition-colors shadow-lg shadow-slate-900/20 flex items-center justify-center gap-2 group"
        >
...
import { ShoppingBasket, X, ArrowRight, Trash2, Plus, Minus } from 'lucide-vue-next'
...
const getItemImage = (item) => {
  ...
  return item.image || item.thumbnail || item.cover_image || '/media-placeholder.jpg'
}
...
const handleBrowseStore = () => {
  ...
  const username = router.currentRoute.value.params.username
  if (username) {
    router.push({ name: 'influencer-assets', params: { username } })
  }
}
...
onMounted(() => {
  ...
  eventBus.on('cart-item-added', fetchCart)
  eventBus.on('cart-item-removed', fetchCart)
  eventBus.on('cart-updated', fetchCart)
})

// Cleanup event listeners
onBeforeUnmount(() => {
  eventBus.off('cart-item-added', fetchCart)
  eventBus.off('cart-item-removed', fetchCart)
